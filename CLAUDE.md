...
| **Tài liệu API công khai** | `docs/api/index.html` — Swagger UI tĩnh (swagger-ui-dist qua jsDelivr) đọc `docs/api/openapi.yaml`, chạy trên **GitHub Pages** (Settings → Pages → Deploy from a branch → `main` + thư mục `/docs`), địa chỉ `https://b23dcat011.github.io/IOT/api/`. `docs/.nojekyll` để Pages phục vụ nguyên văn. ⚠️ **Xuất lại schema mỗi khi API đổi**: `backend/.venv/Scripts/python.exe manage.py spectacular --validate --fail-on-warn --file ../docs/api/openapi.yaml`. Trang này **đã tắt Try it out** (`supportedSubmitMethods: []`) vì máy chủ chỉ chạy nội bộ — muốn gọi thật thì bật `daphne` rồi dùng `http://localhost:8000/api/schema/swagger-ui/`. Dải chú thích màu xanh phía trên đã bỏ theo yêu cầu (24/09) — **đừng thêm lại** |
| **Link trang Profile** | `LINK_GITHUB` = kho ở trên · `LINK_API_DOCS` = trang Pages ở trên (không còn `localhost:8000`) · `LINK_REPORT_PDF` = `.../blob/main/docs/BaoCao.pdf` (đã kiểm: HTTP 200, repo đang **public**) · `LINK_FIGMA` = `figma.com/design/2oR8lFM04tyK7kjpxGrILX/Untitled?node-id=0-1` (thêm 24/09; đã cắt đuôi `?p=f&t=…` vì đó là tham số phiên chia sẻ, hết hạn). ⚠️ **File Figma phải đặt quyền "Anyone with the link → can view"**, không thì thầy bấm vào bị chặn. Sửa link chỉ cần đổi `backend/.env` rồi khởi động lại daphne, **không đụng code**; nhớ sửa kèm `frontend/src/mocks/db.js` cho chế độ dữ liệu giả |
